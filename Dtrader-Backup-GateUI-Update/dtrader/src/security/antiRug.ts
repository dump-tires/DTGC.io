...
import { config, ERC20_ABI, PULSEX_PAIR_ABI } from '../config';
...
export class AntiRugChecker {
   ...
   */
  async checkToken(tokenAddress: string): Promise<TokenSafetyCheck> {
    const warnings: string[] = [];
    let riskScore = 0;

    const token = new ethers.Contract(tokenAddress, EXTENDED_TOKEN_ABI, this.provider);
    ...
    try {
      ...
      if (!owner) owner = await token.getOwner();
    } catch {
      // No owner function - could be good or bad
    }

    if (owner) {
      isOwnerRenounced = owner === ethers.ZeroAddress ||
    ...
    try {
      await token.isBlacklisted(ethers.ZeroAddress);
      hasBlacklist = true;
      if (!isOwnerRenounced) {
    ...
    if (liquidityPls < ethers.parseEther('1000')) {
  ...
  async checkHoneypot(tokenAddress: string): Promise<{
    ...
    try {
      ...
      const testAmountPls = ethers.parseEther('0.1');

      // Simulate buy
      const buyQuote = await pulsex.getQuoteBuy(tokenAddress, testAmountPls, 50);
      ...
      try {
        const sellQuote = await pulsex.getQuoteSell(tokenAddress, buyQuote.amountOut, 50);
   ...
   */
  async checkLiquidityLocked(pairAddress: string): Promise<boolean> {
    try {
   ...
   */
  private async tryGetMaxWallet(token: ethers.Contract): Promise<bigint | null> {
   ...
   */
  private async tryGetMaxTx(token: ethers.Contract): Promise<bigint | null> {
   ...
   */
  async quickHoneypotCheck(tokenAddress: string): Promise<boolean> {
...
💧 **Liquidity**
${ethers.formatEther(check.liquidityPls)} PLS
    `.trim();
...
export const antiRug = new AntiRugChecker();
