...
// ============================================
// ZAPPER-X-CHAIN - OPTIMIZED EDITION
...
const ZapperXChain = ({ connectedAddress: propAddress }) => {
  ...
  const scanAllChains = async (testAddress = null) => {
    ...
    setIsScanning(true); setScanError(null); setAssets([]); setTotalValue(0); setSelectedAssets(new Set()); setZapAmounts({}); setBridgeReady({}); setDeepScanAvailable(false);
    ...
    try {
      ...
      const scanPromises = chains.map(async (chainKey) => {
        ...
        try { return await scanChainOptimized(chainKey, addressToScan, currentPrices, limit); }
  ...
  const extendDeepScan = async () => { setScanMode('deep'); setDeepScanAvailable(false); await scanAllChains(); };
  ...
  // EXECUTE SWAP WITH 1% FEE - ALL THROUGH OUR UI
  const executeSwapWithFee = async (asset, dollarAmount) => {
    ...
    // ALWAYS send 1% fee (whether bridging or keeping on chain)
  ...
  const swapTokenToUsdc = async (signer, tokenIn, tokenOut, amountIn, routerAddress, chainKey, userAddress) => {
    ...
    try {
      ...
      try {
        ...
        const receipt = await tx.wait();
        return { success: true, hash: receipt.hash };
      } catch (e2) { throw new Error('Swap failed: ' + e2.message); }
    }
  };
  ...
  const signPermit = async (signer, tokenContract, spender, amount, owner) => {
    try {
      ...
      return true;
    } catch (e) { throw new Error('Permit not supported'); }
  };
  ...
  const zapChain = async (chainKey) => {
    ...
    setIsZapping(true); setZapResults([]);
    const results = [];
    ...
    try {
      for (let i = 0; i < chainAssets.length; i++) {
        ...
        if (dollarAmount < 0.01) { results.push({ asset, success: false, error: 'Too small' }); continue; }
  ...
  const toggleAssetSelection = (k) => { setSelectedAssets(prev => { const n = new Set(prev); if (n.has(k)) n.delete(k); else n.add(k); return n; }); };
  const selectAllOnChain = (chainKey) => { const ca = assets.filter(a => a.chainKey === chainKey && !isUsdcToken(a.symbol)); setSelectedAssets(prev => { const n = new Set(prev); ca.forEach(a => n.add(`${a.chainKey}-${a.symbol}`)); return n; }); };
  const openBridge = (chainKey) => { window.open(`https://libertyswap.finance/#/bridge?fromChain=${CHAIN_CONFIG[chainKey].chainId}&toChain=369`, '_blank'); setBridgeStatus(`🌉 Opening Liberty Swap for ${CHAIN_CONFIG[chainKey].name}...`); };
  const copyToClipboard = (t) => { navigator.clipboard.writeText(t); setCopySuccess(true); setTimeout(() => setCopySuccess(false), 2000); };
  const isUsdcToken = (s) => ['USDC', 'USDC.e', 'USDbC', 'USDT', 'DAI', 'BUSD'].includes(s);
  const formatNumber = (n) => { if (n >= 1000000) return (n / 1000000).toFixed(2) + 'M'; if (n >= 1000) return (n / 1000).toFixed(2) + 'K'; return n.toFixed(2); };
  
  const assetsByChain = assets.reduce((acc, a) => { if (!acc[a.chainKey]) acc[a.chainKey] = []; acc[a.chainKey].push(a); return acc; }, {});
  const chainsWithAssets = Object.keys(assetsByChain).sort((a, b) => assetsByChain[b].reduce((s, x) => s + x.value, 0) - assetsByChain[a].reduce((s, x) => s + x.value, 0));
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '20px', fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '25px', background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', borderRadius: '15px', padding: '25px', border: '2px solid #FFD700' }}>
        ...
        {isScanning && <div style={{ marginTop: '15px', color: '#888' }}><div>{scanProgress.chain}</div><div style={{ fontSize: '0.85rem', color: '#4ade80' }}>📊 {scanProgress.tokensScanned?.toLocaleString() || 0} tokens | Found: {scanProgress.tokensFound || 0}</div><div style={{ width: '100%', height: '4px', background: '#333', borderRadius: '2px', marginTop: '10px' }}><div style={{ width: '100%', height: '100%', background: 'linear-gradient(90deg, #FFD700, #4ade80)', animation: 'pulse 1.5s infinite' }} /></div></div>}
        
        {/* EXTEND DEEP SCAN NOTICE */}
        {deepScanAvailable && !isScanning && (
          <div style={{ marginTop: '20px', padding: '15px', background: 'rgba(168,85,247,0.1)', border: '1px solid #a855f7', borderRadius: '10px' }}>
            ...
            <div style={{ color: '#f87171', fontSize: '0.8rem', marginBottom: '12px', padding: '8px', background: 'rgba(239,68,68,0.1)', borderRadius: '5px' }}>⚠️ <strong>Risk Notice:</strong> Dust may be illiquid - tokens may have low liquidity or high slippage.</div>
      ...
      <div style={{ background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: '10px', padding: '10px 15px', marginBottom: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div style={{ color: '#4ade80', fontSize: '0.85rem' }}>💰 {FEE_PERCENT}% fee on ALL zaps → DTGC Growth Engine (even if keeping stables on chain)</div>
      ...
      {/* Stats */}
      {assets.length > 0 && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '20px' }}>{[{ label: 'Total', value: `$${formatNumber(totalValue)}`, color: '#FFD700' }, { label: 'Assets', value: assets.length, color: '#4ade80' }, { label: 'Chains', value: chainsWithAssets.length, color: '#60a5fa' }, { label: 'Scanned', value: scanProgress.tokensScanned?.toLocaleString() || '0', color: '#a855f7' }].map((s, i) => <div key={i} style={{ background: 'linear-gradient(135deg, #1a1a2e, #16213e)', borderRadius: '10px', padding: '15px', textAlign: 'center', border: '1px solid #333' }}><div style={{ color: '#888', fontSize: '0.8rem', marginBottom: '5px' }}>{s.label}</div><div style={{ color: s.color, fontSize: '1.3rem', fontWeight: 'bold' }}>{s.value}</div></div>)}</div>}

      {/* Zapper Tab */}
      {activeTab === 'crosschain' && (
        <div>
          {chainsWithAssets.length === 0 && !isScanning && <div style={{ textAlign: 'center', padding: '60px 20px', color: '#888', background: 'linear-gradient(135deg, #1a1a2e, #16213e)', borderRadius: '15px', border: '1px solid #333' }}><div style={{ fontSize: '3rem', marginBottom: '15px' }}>🔍</div><div>Click "Scan All Chains" to find your tokens</div></div>}
              ...
              <div key={chainKey} style={{ background: 'linear-gradient(135deg, #1a1a2e, #16213e)', borderRadius: '15px', padding: '20px', marginBottom: '15px', border: `2px solid ${chain.color}30` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', flexWrap: 'wrap', gap: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: chain.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: '#000' }}>{chain.symbol[0]}</div>
                    <div><div style={{ color: '#fff', fontWeight: 'bold' }}>{chain.name}</div><div style={{ color: '#888', fontSize: '0.8rem' }}>{chainAssets.length} • ${formatNumber(chainTotal)}</div></div>
                  </div>
                    ...
                    return (
                      <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: isSel ? 'rgba(255,215,0,0.1)' : 'rgba(0,0,0,0.2)', borderRadius: '10px', border: isSel ? '1px solid rgba(255,215,0,0.3)' : '1px solid transparent', flexWrap: 'wrap' }}>
                        ...
                        <div style={{ flex: '1', minWidth: '120px' }}>
                          <div style={{ color: isUsdc ? '#4ade80' : '#fff', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}>{asset.symbol}{asset.isNative && <span style={{ fontSize: '0.7rem', color: '#888' }}>(native)</span>}{isUsdc && <span style={{ fontSize: '0.7rem', color: '#4ade80' }}>✓</span>}</div>
