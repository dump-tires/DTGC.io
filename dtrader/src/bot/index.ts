...
🌐 Web: dtgc.io/gold`;
  ...
  private async handleCallback(chatId: string, userId: string, data: string, messageId: number): Promise<void> {
    ...
    if (data === 'backup_info') {
      ...
      if (linkedWallet) msg += `• ✅ Gold wallet link\n`;
      msg += `\n`;
    ...
    if (data === 'help_instabond') {
      await this.bot.sendMessage(chatId,
        ...
        `• Higher bonding = safer but less upside`,
        { parse_mode: 'Markdown', reply_markup: keyboards.helpMenuKeyboard }
      );
      return;
    }
    ...
    if (data === 'help_orders') {
      await this.bot.sendMessage(chatId,
        ...
        `• Orders stay active until executed or cancelled`,
        { parse_mode: 'Markdown', reply_markup: keyboards.helpMenuKeyboard }
      );
      return;
    }
    ...
    if (data === 'help_wallets') {
      await this.bot.sendMessage(chatId,
        ...
        `• Never put large amounts in bot wallet`,
        { parse_mode: 'Markdown', reply_markup: keyboards.helpMenuKeyboard }
      );
      return;
    }
    ...
    if (data.startsWith('instabond_tp_') && !data.includes('custom') && !data.includes('none')) {
      ...
      await this.bot.sendMessage(chatId,
        ...
        `💡 At +100% (2x), selling 50% recovers your initial investment!`,
        { parse_mode: 'Markdown', reply_markup: keyboards.pumpMenuKeyboard }
      );
      return;
    }
  ...
  private async handleTextInput(chatId: string, userId: string, text: string): Promise<void> {
    ...
    if (session.pendingAction === 'link_gate_wallet') {
      ...
      if (keyLast4.length !== 4) {
        await this.bot.sendMessage(chatId,
          `❌ Please enter exactly 4 characters.\n\n` +
          `Example: \`a1b2\``,
          { parse_mode: 'Markdown' }
        );
        return;
    ...
    if (session.pendingAction === 'post_verify_recover') {
      ...
      if (result) {
        ...
        await this.bot.sendMessage(chatId,
          ...
          `💡 Try a different wallet's private key, or use /recover for more options.`,
          { parse_mode: 'Markdown' }
        );
      }
      return;
    ...
    if (session.pendingAction === 'limit_buy_price') {
      ...
      if (text.includes('%')) {
        ...
        if (!currentPrice) {
          await this.bot.sendMessage(chatId,
            `❌ No price data available for percentage calculation.\n\n` +
    ...
    if (session.pendingAction === 'limit_buy_amount') {
      ...
      const walletListStr = walletsWithBalance.map(w =>
        `${w.selected ? '🟢' : '⚪'} #${w.index} ${w.label || 'Wallet ' + w.index}: **${w.balanceFormatted} PLS**`
      ).join('\n');

      await this.bot.sendMessage(chatId,
        `👛 **Select Wallet(s) for Limit Order**\n` +
        `━━━━━━━━━━━━━━━━━━━━━\n\n` +
        `📊 Order: **LIMIT BUY**\n` +
        `🪙 Token: \`${session.pendingToken?.slice(0, 12)}...\`\n` +
  ...
  private async showWalletBalances(chatId: string, userId: string): Promise<void> {
    ...
    if (session.linkedWallet) {
      try {
        ...
        if (orders > 0) msg += `📋 ${orders} pending order${orders > 1 ? 's' : ''}\n`;
        msg += `\n`;
      } catch (e) {
  ...
  private setupSniperEvents(): void {
    ...
    graduationSniper.on('snipeReady', async (data) => {
      ...
      if (!activeWallet) {
        ...
        await this.bot.sendMessage(chatId,
          `❌ **Snipe Failed**\n\nNo snipe wallet found. Generate wallets first with /wallets → Generate 6 New.`,
          { parse_mode: 'Markdown' }
        );
        return;
      }
    ...
    limitOrderEngine.on('orderTriggered', async (data) => {
      ...
      if (userChatId) {
        await this.bot.sendMessage(userChatId,
          ...
          `📈 Current: ${ethers.formatEther(priceData.priceInPls)} PLS\n\n` +
          `⏳ Executing trade with wallet ${order.walletAddress.slice(0, 8)}...`,
          { parse_mode: 'Markdown' }
        );
      }
    ...
    try {
      ...
      if (probableWins.length > 0) {
        ...
        for (let i = 0; i < Math.min(3, probableWins.length); i++) {
          ...
          winsMsg += `   💧 $${dexScreener.formatNumber(t.liquidity)} | ${t.priceChange24h >= 0 ? '📈' : '📉'} ${t.priceChange24h >= 0 ? '+' : ''}${t.priceChange24h.toFixed(1)}%\n`;
        }
  ...
  private async showProbableWins(chatId: string, userId: string): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(probableWins.length, 10); i++) {
        ...
        msg += `   💧 $${dexScreener.formatNumber(t.liquidity)} Liq | `;
        msg += `📊 $${dexScreener.formatNumber(t.volume24h)} Vol | `;
