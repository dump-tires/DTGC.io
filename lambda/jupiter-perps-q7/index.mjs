...
// ═══════════════════════════════════════════════════════════════════════════════
/**
 * Get wallet public key from configured private key
 * @returns {string|null} Base58 public key string or null if not configured
 */
function getWalletPublicKey() {
  ...
  try {
    ...
    if (CONFIG.PRIVATE_KEY.startsWith('[')) {
      ...
      secretKey = bs58.default.decode(CONFIG.PRIVATE_KEY);
    }

    const keypair = Keypair.fromSecretKey(secretKey);
    return keypair.publicKey.toBase58();
  } catch (e) {
    log('error', 'Failed to derive wallet public key', { error: e.message });
    return null;
  }
}

/**
 * Get Keypair from configured private key (for signing transactions)
 * @returns {Keypair|null} Solana Keypair or null if not configured
 */
function getWalletKeypair() {
  ...
  try {
    ...
    log('error', 'Failed to create wallet keypair', { error: e.message });
    return null;
  }
}
...
// ═══════════════════════════════════════════════════════════════════════════════
/**
 * Fetch open positions from Jupiter Perps API
 * @returns {Array} Array of position objects
 */
async function getJupiterPositions() {
  const walletPubkey = getWalletPublicKey();
  if (!walletPubkey) {
    log('warn', 'No wallet configured, cannot fetch positions');
    return [];
  }

  try {
    const url = `${CONFIG.JUPITER_PERPS_API}/v1/positions?walletAddress=${walletPubkey}`;
    const response = await httpsGet(url);

    if (!response || !response.dataList) {
      log('warn', 'Invalid response from Jupiter positions API');
      return [];
    }

    // Map Jupiter API response to our position format
    return response.dataList.map(pos => ({
      asset: pos.asset,
      side: pos.side,
      long: pos.side === 'long',
      sizeUsd: parseFloat(pos.sizeUsd) / 1e6,
      collateral: parseFloat(pos.collateralUsd) / 1e6,
      collateralUsd: parseFloat(pos.collateralUsd) / 1e6,
      leverage: parseFloat(pos.leverage),
      entryPrice: parseFloat(pos.entryPriceUsd) / 1e6,
      markPrice: parseFloat(pos.markPriceUsd) / 1e6,
      liquidationPrice: parseFloat(pos.liquidationPriceUsd) / 1e6,
      pnlUsd: parseFloat(pos.pnlAfterFeesUsd) / 1e6,
      pnlPct: parseFloat(pos.pnlBeforeFeesPct),
